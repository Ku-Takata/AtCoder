...
#include <algorithm>
#include <chrono>
#include <unordered_set> // ★ 重複排除のために追加
...
uint64_t zobrist_table[20][20][405];

// ★ リスタートごとに多様性を持たせるための動的評価パラメータ
long long run_weights[4] = {10000LL, 5000LL, 1000LL, 100LL};
long long eval_w[4] = {120, 80, 40, 10};
int random_noise_amp = 8000;
...
void init_zobrist() {
    for (int r = 0; r < 20; ++r) {
        for (int c = 0; c < 20; ++c) {
            for (int v = 0; v < 405; ++v) {
                uint64_t high = xor128();
                uint64_t low = xor128();
                zobrist_table[r][c][v] = (high << 32) | low;
            }
        }
    }
}

// ★ 盤面状態から高速にハッシュ値を計算する関数
uint64_t calc_hash(const State& st) {
    uint64_t h = 0;
    for (int r = 0; r < 20; ++r) {
        for (int c = 0; c < 20; ++c) {
            int v = st.grid[r][c];
...
State run_macro_step(const State& parent_state) {
        ...
        for (int i = 0; i < cb_cnt[tr][tc]; ++i) {
            ...
            for (int d_idx = 0; d_idx < 2; ++d_idx) {
                ...
                for (int offset = 1; offset <= 4; ++offset) {
                    ...
                    sub_score += run_weights[offset - 1] * dist_to_exit[cr][cc];
                }

                if (random_noise_amp > 0) {
                    sub_score += xor128() % random_noise_amp;
                }
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (t < 400 && child.pos_r[t] != -1) {
            child.eval_score += eval_w[i] * dist_to_exit[child.pos_r[t]][child.pos_c[t]];
        }
    }

    // ★ 順序逆転ペナルティ
    for (int i = 0; i < 5; ++i) {
        int t1 = child.current_target + i;
        if (t1 >= 400 || child.pos_r[t1] == -1) continue;
        for (int j = i + 1; j < 5; ++j) {
            int t2 = child.current_target + j;
            if (t2 >= 400 || child.pos_r[t2] == -1) continue;
            if (dist_to_exit[child.pos_r[t1]][child.pos_c[t1]] > dist_to_exit[child.pos_r[t2]][child.pos_c[t2]]) {
                child.eval_score += 40LL; 
            }
        }
    }

    // ★ 新設：出口付近の「目詰まり」ペナルティ
    // 直近に出す予定のない大きな番号の箱が出口のすぐ近く（距離3以内）をうろついている場合、進路妨害リスクとして評価値を悪化させる
    for (int t = child.current_target + 1; t < min(400, child.current_target + 8); ++t) {
        if (child.pos_r[t] != -1) {
            int d = dist_to_exit[child.pos_r[t]][child.pos_c[t]];
            if (d <= 3) {
                child.eval_score += (4 - d) * 35LL; 
            }
        }
    }
...
int main() {
    ...
    // 高速化のためunordered_setのバッファをループ外で1度だけ確保
    unordered_set<uint64_t> seen_hashes;
    seen_hashes.reserve(250);
    ...
    while (true) {
        ...
        // Chilled Beam Search的アプローチ：リスタートごとに評価重みをランダムに変調
        run_weights[0] = 6000LL + (xor128() % 8000);
        run_weights[1] = 3000LL + (xor128() % 4000);
        run_weights[2] = 600LL + (xor128() % 800);
        run_weights[3] = 60LL + (xor128() % 80);

        eval_w[0] = 80LL + (xor128() % 80);
        eval_w[1] = 40LL + (xor128() % 60);
        eval_w[2] = 20LL + (xor128() % 30);
        eval_w[3] = 5LL + (xor128() % 10);

        random_noise_amp = 4000 + (xor128() % 8000);
        ...
        while (true) {
            ...
            // ★ 変更：Zobrist Hashを用いた同一盤面の重複排除（ユニーク化）
            vector<State> unique_candidates;
            unique_candidates.reserve(BEAM_WIDTH);
            seen_hashes.clear();

            for (const auto& st : next_candidates) {
                uint64_t h = calc_hash(st);
                if (seen_hashes.count(h)) continue; // すでに同じ盤面がビーム内にあればスキップ
                seen_hashes.insert(h);
                unique_candidates.push_back(st);
                if (unique_candidates.size() >= BEAM_WIDTH) break;
            }
            current_beam = move(unique_candidates);
        }
