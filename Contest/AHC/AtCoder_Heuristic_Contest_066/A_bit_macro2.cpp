...
inline int get_dist_to_target(int r, int c, int holding, uint64_t completed) {
    if (holding != -1) {
        return dist_basket[holding][r][c];
    } else {
        int min_d = 1e9;
        for (int k = 0; k < M; ++k) {
            if (!(completed & (1ULL << k))) {
...
inline int evaluate(int r, int c, int holding, uint64_t completed, bool is_rec, uint64_t reg_macro, int step) {
    ...
    if ((reg_macro & 0x1F) > 0) score += (reg_macro & 0x1F); 
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (sim_base(nr, nc, ndir, nhold, ncomp, i)) {
                    ...
                    if (s.is_rec) {
                        ...
                        // 🎯【改善】ターゲットまでの距離に対して「+5」のあそびを持たせる
                        int dist_to_target = get_dist_to_target(s.r, s.c, s.holding, s.completed);
                        if (cur_len > dist_to_target + 5) continue; 
                ...
                if (ok) {
                    ...
                    if (s.is_rec) {
                        ...
                        if (new_len <= MAX_MACRO_LEN) { 
                            // 🎯【改善】P再生時も同様に「+5」のあそびで判定
                            int dist_to_target = get_dist_to_target(s.r, s.c, s.holding, s.completed);
                            if (new_len > dist_to_target + 5) {
                                ok = false;
                            } else {
