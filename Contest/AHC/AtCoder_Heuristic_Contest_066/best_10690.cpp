    ...
    for (int step = 1; step <= T; ++step) {
        ...
        for (const State& s : current_beam) {
            auto try_push = [&](int8_t nr, int8_t nc, int8_t ndir, int8_t nhold, uint64_t ncomp, 
                                uint64_t nreg, uint64_t nrec, bool nrec_f, char op, int nreal) {
                if (nreal > T) return; 

                State ns = {ncomp, nreg, nrec, 0, s.history_idx, nreal, nr, nc, ndir, nhold, nrec_f, op};
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (sim_base(nr, nc, ndir, nhold, ncomp, i)) {
                    ...
                    try_push(nr, nc, ndir, nhold, ncomp, s.reg_macro, nrec, s.is_rec, op, s.real_cost + 1);
                }
            }

            if (s.temp_op != 'M') { 
                // 🎯 録画を開始してもマクロ長上限分（20手）の予算が残っていなければMを禁止
                if (!s.is_rec && (T - s.real_cost < MAX_MACRO_LEN)) {
                    ...
                    try_push(s.r, s.c, s.dir, s.holding, s.completed, nreg, nrec, nrec_f, 'M', s.real_cost);
                }
            }
            ...
            if ((s.reg_macro & 0x1F) > 0) { 
                ...
                if (ok) {
                    ...
                    if (ok) try_push(nr, nc, ndir, nhold, ncomp, s.reg_macro, nrec, s.is_rec, 'P', s.real_cost + m_len);
                }
            }
        }
...
END_SEARCH:
    // 🎯 ゴール到達済みの場合はその履歴を、時間切れ等の場合は部分点の履歴を使用
    ...
    // 🎯 どちらの状態を出力したか、デバッグ出力で明確に分離
    if (best_goal_history != -1) {
        cerr << "[DEBUG] Goal Reached! cmd_len: " << final_ops.length() 
             << " / expanded_real_cost: " << best_goal_real_cost << "\n";
    } else {
        cerr << "[DEBUG] Partial Output. cmd_len: " << final_ops.length() 
