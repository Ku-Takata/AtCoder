...
string solve_greedy() {
    ...
    ops += 'M';
    for(int i = 0; i < 8; i++) { ops += 'F'; apply_F(); } // 🎯 4手から8手へ変更
    ops += 'M';

    while (completed != ((1ULL << M) - 1)) {
        if (real_cost > T || static_cast<int>(ops.length()) > T) break; // 🎯 警告対策キャスト
        ...
        while (r != target_r || c != target_c) {
            ...
            if (straight_len >= 8) { // 🎯 4マスから8マスへ変更
                ops += 'P';
                for (int i = 0; i < 8; i++) apply_F(); // 🎯 P展開時も8歩進むように変更
            } else {
                ops += 'F';
                apply_F();
            }
    ...
    if (static_cast<int>(ops.length()) > T) ops = ops.substr(0, T); // 🎯 警告対策キャスト
    return ops;
}
...
int main() {
    ...
    double TIME_LIMIT = 1.95; // 🎯 1.95に設定
    ...
    const int BEAM_WIDTH = 8000; // 🎯 8000に設定
    
    // 🎯 未使用変数警告対策の属性を追加
    [[maybe_unused]] int best_partial_history = 0;
    [[maybe_unused]] int best_partial_score = -1e9;
    ...
    for (int step = 1; step <= T; ++step) {
        ...
        for (const State& s : current_beam) {
            ...
            for (int i = 0; i < 4; ++i) {
                if (force_macro_end) continue;     
                if (s.is_rec && i == 3) continue;  
            ...
            if (!force_macro_end && (s.reg_macro & 0x1F) > 0) { 
                ...
                // 🎯 警告対策カウンタの型を uint64_t に変更
                for (uint64_t k = 0; k < m_len; ++k) {
