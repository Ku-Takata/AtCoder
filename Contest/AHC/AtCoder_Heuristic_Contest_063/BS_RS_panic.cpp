...
struct State {
    ...
    long long _calc_score() {
        ...
        // 300ターンスタックしている場合、自身の胴体(3/4位置)をターゲットとして強烈に誘導する
        int stagnant_turns = moves_length - last_match_turn;
        if (stagnant_turns > 300) {
            // ▼▼▼ ここから変更 ▼▼▼
            
            // 正解の色を失わないための安全な切断ライン (これよりインデックスが大きい＝後ろを噛めばセーフ)
            int safe_cut_idx = matched + 1;
            
            // 本来噛みちぎりたい理想の長さ(現在の長さの3/4)
            int target_idx = (snake_len * 3) / 4;

            // もし理想の位置を噛むと正解の色まで失ってしまう場合
            if (target_idx <= safe_cut_idx) {
                // 【モードA: バッファ構築モード】
                // 噛みちぎるための「捨ててもいい尻尾（ゴミ）」が足りないので、
                // 一番近い餌（何色でもOK）を食べて意図的に胴体を伸ばす
                int head_r = pos_r[snake[0]];
                int head_c = pos_c[snake[0]];
                int nearest_any_dist = 10000;
                
                // 盤面上のすべての餌から最短のものを探す
                for (int c_id = 1; c_id <= C; ++c_id) {
                    for (int p : bait_positions[c_id]) {
                        if (board[p] > 0) { // 餌がまだ盤面にある
                            int dist = abs(head_r - pos_r[p]) + abs(head_c - pos_c[p]);
                ...
                if (nearest_any_dist != 10000) {
                    // 通常の餌探索よりも圧倒的に優先して適当な餌に向かわせる
                    score_val -= nearest_any_dist * 50000LL;
                }
            } else {
                // 【モードB: 安全な噛みちぎりモード】
                // 十分にゴミが溜まった（target_idx が safe_cut_idx より後ろになった）ので、
                // 正解の色を傷つけない位置を噛みちぎる
        ...
        if (garbage > 0) {
            score_val -= garbage * current_garbage_penalty;
            if (matched >= 2) {
                ...
                score_val -= perceived_dist_bite * current_bite_penalty;
            }
        }
    ...
    State() : snake_len(0), colors_len(0), node_id(-1), moves_length(0), matched(0), garbage(0), score(0), last_match_turn(0) {}
...
string beam_search(int beam_width, int max_turns, double time_limit, int noise_max) {
        ...
        for (State& state : beam) {
            ...
            for (int d_idx = 0; d_idx < 4; ++d_idx) {
                ...
                if (bite_idx_new != -1) {
                    ...
                    if (nxt_state.matched > state.matched || state.moves_length - state.last_match_turn > 300) {
    ...
    if (best_state.matched < M || best_state.garbage > 0) {
        if (debug_fail_reason == "") debug_fail_reason = "Not Finished";
        return "";
    }
    // -------------------------------------------------------------------------
