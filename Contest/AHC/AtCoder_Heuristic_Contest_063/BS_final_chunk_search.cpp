...
int dist_to_color[MAX_N * MAX_N][MAX_N * MAX_N];
...
struct State {
    uint8_t snake[MAX_N * MAX_N];
    int16_t snake_len;
    
    uint8_t colors[MAX_N * MAX_N];
    int16_t colors_len;
    
    uint8_t board[MAX_N * MAX_N];
    ...
    long long _calc_score() {
        ...
        if (stagnant_turns > panic_limit) {
            ...
            if (bite_idx > snake_len - 3) {
                ...
                // ゴミを完全に切り落とすための長さ（バッファ）がまだ足りない。
                // ペナルティを無視して「次の正解の密集地」へ強引に突進し、食べながら伸ばす。
                is_bulldozer = true;
            } else {
                // 【モードB: 完璧な噛みちぎりモード】
        ...
        if (garbage > 0) {
            ...
            // ▼ 変更: matched >= 2 の条件を撤廃し、常に完璧な切断位置をターゲットにする
...
struct SearchResult {
    ...
    SearchResult(const string& m, const State& s) : moves(m), state(s) {}
};
...
struct RouteCandidate {
    ...
    RouteCandidate(const string& m, const State& s) : moves(m), state(s) {}
};

using StateKey = uint64_t;

SearchResult beam_search(const State& start_state, int target_match, int beam_width, int max_turns, double time_limit, int noise_max) {
    ...
    // ↓↓↓ 変更: 外部から受け取った状態を初期状態とする ↓↓↓
    State initial_state = start_state;
    initial_state.node_id = 0; // この区間のルートとしてリセット
    initial_state.moves_length = 0;
    initial_state.last_match_turn = 0;
    
    vector<State> beam = {initial_state};
    State best_state = initial_state;
    
    // --- 変更箇所: vectorの代わりに固定長プールとインデックス配列を用意 ---
    State next_beam_pool[400];
    pair<long long, int> next_beam_info[400];
    StateKey next_beam_keys[400]; // --- 追加: 重複判定用のキー配列
    ...
    for (int turn = 0; turn < max_turns; ++turn) {
        ...
        for (State& state : beam) {
            if (state.matched >= target_match && state.garbage == 0) {
                if (best_state.matched < target_match || best_state.garbage > 0 || state.moves_length < best_state.moves_length) {
                    best_state = state;
                }
                continue;
            }
            ...
            for (int d_idx = 0; d_idx < 4; ++d_idx) {
                ...
                if (bite_idx_new != -1) {
                    ...
                    // ↓↓↓ 追加: 区間目標に到達した状態を確実に行き残らせる ↓↓↓
                    if (nxt_state.matched >= target_match && nxt_state.garbage == 0) {
                        nxt_state.score += 10000000LL;
                    }
                    ...
                    // 変更: tuple ではなく uint64_t にビットシフトで高速パックする
                    StateKey state_key = 0;
                    state_key |= (uint64_t)nxt_state.snake[0];
                    state_key |= ((uint64_t)nxt_state.snake[nxt_state.snake_len - 1]) << 10;
                    state_key |= ((uint64_t)nxt_state.snake_len) << 20;
                    state_key |= ((uint64_t)nxt_state.matched) << 30;
                    state_key |= ((uint64_t)nxt_state.garbage) << 40;
                    state_key |= ((uint64_t)nxt_state.snake[mid_idx]) << 50;
        ...
        if (best_state.matched >= target_match && best_state.garbage == 0) {
            break;
        }
    }
    // --- 変更: 最終的なベスト状態の情報を記録 ---
    ...
    // --- 追加箇所: 探索が終わった時点で目標を達成できていなければ失敗とする ---
    if (best_state.matched < target_match || best_state.garbage > 0) {
        if (debug_fail_reason == "") {
            if (max_turns != 100000) debug_fail_reason = "Pruned by Best Score";
            else debug_fail_reason = "Not Finished";
        }
        return {"", best_state}; // 空文字列と状態を返す
    }
    ...
    return {best_moves, best_state}; // 文字列と到達状態をセットで返す
}
...
int main() {
    ...
    // --- 追加: 探索の初期状態(ルート状態)をあらかじめ作成 ---
    vector<int> initial_snake(5);
    for (int i = 0; i < 5; ++i) initial_snake[i] = (4 - i) * N;
    vector<int> initial_colors = {1, 1, 1, 1, 1};
    State root_state;
    root_state.init(initial_snake, initial_colors, mass_flat);

    // ----------------------------------------------------
    // 【第1段階】偵察（プローブ）実行
    // ----------------------------------------------------
    cerr << "=== [Phase 1] Probe Start (0.3s) ===\n";
    // 変更: root_state と M を渡すように変更
    SearchResult probe_res = beam_search(root_state, M, 30, 100000, 0.3, 500);
    string probe_ans = probe_res.moves;
    ...
    cerr << "====================================\n\n";
    cerr << "=== [Phase 2] Main Search Start (Chunked) ===\n";

    // ----------------------------------------------------
    // 【第2段階】区間ごとに上位3ルートを保持しながらリセマラ
    // ----------------------------------------------------
    vector<RouteCandidate> current_candidates;
    current_candidates.push_back(RouteCandidate{"", root_state}); // 初期状態をセット

    int chunk_size = 100; // 1区間あたりの目標進行度
    int num_chunks = (M + chunk_size - 1) / chunk_size;
    
    // 残り時間を各区間に均等に配分
    double current_total_time = chrono::duration<double>(chrono::high_resolution_clock::now() - start_time).count();
    double time_per_chunk = max(0.01, (1.95 - current_total_time) / num_chunks);

    for (int target = chunk_size; target <= M + chunk_size; target += chunk_size) {
        int current_target = min(target, M);
        cerr << "--- Target Chunk: " << current_target << " / " << M << " ---\n";
        
        vector<RouteCandidate> next_candidates;
        auto chunk_start_time = chrono::high_resolution_clock::now();
        int trial_count = 1;

        while (true) {
            auto current_time = chrono::high_resolution_clock::now();
            double elapsed_in_chunk = chrono::duration<double>(current_time - chunk_start_time).count();
            
            // 区間の持ち時間を超えたら次の区間へ
            if (elapsed_in_chunk > time_per_chunk) break;
            
            // 全体の制限時間を超えても終了
            double total_elapsed = chrono::duration<double>(current_time - start_time).count();
            if (total_elapsed > 1.95) break;

            double attempt_end_time = total_elapsed + min(0.3, time_per_chunk - elapsed_in_chunk);
            ...
            // current_candidates (上位3ルート) からランダムに親ルートを選ぶ
            int parent_idx = mt_engine() % current_candidates.size();
            const RouteCandidate& parent = current_candidates[parent_idx];

            // --- 変更: 1区間あたりの足切りターン数を厳密に設定する ---
            // 1色あたり平均 20 手を上限とする (chunk_size = 40 なら 800 手)
            // これを超えた探索は「ハズレルート」として即座に打ち切り、次の乱数を試す
            int max_allowed_turns = (current_target - parent.state.matched) * 30;
            
            if (min_moves != 1000000) {
                int remaining_budget = min_moves - parent.moves.length();
                max_allowed_turns = min(max_allowed_turns, remaining_budget);
                if (max_allowed_turns <= 0) continue;
            }

            // ↓↓↓ 追加: 既に5つ以上の成功ルートが見つかっている場合、暫定3位の記録で足切りする ↓↓↓
            if (next_candidates.size() >= 5) {
                vector<int> current_lens;
                for (const auto& cand : next_candidates) {
                    current_lens.push_back(cand.moves.length());
                }
                // ターン数が少ない順にソート
                sort(current_lens.begin(), current_lens.end());
                
                // 最下位の総ターン数
                int lowest_total = current_lens[4];
                
                // この試行で許容される最大ターン数 = 暫定3位の総ターン数 - 親ルートがすでに消費したターン数
                int dynamic_limit = lowest_total - parent.moves.length();
                
                // 親の時点ですでに3位以上のターン数を消費している場合は、探索する意味がないのでスキップ
                if (dynamic_limit <= 0) continue;
                
                // 上限をさらに厳しいほうに更新する
                max_allowed_turns = min(max_allowed_turns, dynamic_limit);
            }

            auto start_search = chrono::high_resolution_clock::now();
            SearchResult res = beam_search(parent.state, current_target, current_beam, max_allowed_turns, attempt_end_time, current_noise);
            auto end_search = chrono::high_resolution_clock::now();
            
            if (!res.moves.empty()) {
                next_candidates.push_back(RouteCandidate{parent.moves + res.moves, res.state});
                cerr << "Trial " << trial_count << " | Reached! (Chunk length: " << res.moves.length() << ")\n";
                
                if (current_target == M) {
                    int total_len = parent.moves.length() + res.moves.length();
                    if (total_len < min_moves) {
                        best_ans = parent.moves + res.moves;
                        min_moves = total_len;
                        cerr << "  <-- Global Best Updated: " << min_moves << " moves\n";
                    }
                }
            }
            trial_count++;
        ...
        if (next_candidates.empty()) {
            cerr << "Failed to reach target " << current_target << " in time.\n";
            break;
        }

        // ターン数でソートし、上位3つ(TOP_K)を残す
        sort(next_candidates.begin(), next_candidates.end(), [](const RouteCandidate& a, const RouteCandidate& b) {
            return a.moves.length() < b.moves.length();
        });

        current_candidates.clear();
        int KEEP_K = 3;
        for (int i = 0; i < min(KEEP_K, (int)next_candidates.size()); ++i) {
            current_candidates.push_back(next_candidates[i]);
        }
        
        if (current_target == M) break;
    }
    cerr << "====================================\n";
