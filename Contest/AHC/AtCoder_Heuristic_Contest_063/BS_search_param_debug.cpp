...
int main() {
    ...
    vector<pair<int, int>> params_hard = {
        {10, 2000}, {15, 1500}, {20, 1500}, {20, 500}, {25, 1000}
    };
    vector<pair<int, int>> params_normal = {
        {35, 500}, {30, 1000}, {25, 1000}, {40, 1000}, {40, 500}
    };
    vector<pair<int, int>> params_easy = {
        {40, 250}, {45, 250}, {40, 500}, {50, 300}, {50, 100}
    };
    ...
    if (probe_ans.empty()) {
        cerr << "Probe Result : Failed (Time out)\n";
        cerr << "Difficulty   : HARD\n";
        cerr << "Selected Pool: params_hard\n";
        current_pool = params_hard;
    } else {
        best_ans = probe_ans;
        min_moves = probe_ans.length();
        cerr << "Probe Result : " << min_moves << " moves\n";
        
        if (min_moves < 2000) {
            cerr << "Difficulty   : EASY\n";
            cerr << "Selected Pool: params_easy\n";
            current_pool = params_easy;
        } else {
            cerr << "Difficulty   : NORMAL\n";
            cerr << "Selected Pool: params_normal\n";
    ...
    while (true) {
        ...
        param_idx = (param_idx + 1) % current_pool.size(); 
        
        string ans = beam_search(current_beam, 100000, attempt_end_time, current_noise);
        
        // --- デバッグ出力: 今回の試行結果 ---
        cerr << "Trial " << trial_count++ << " | Beam: " << current_beam << " | Noise: " << current_noise << " | Result: ";
        if (ans.empty()) {
            cerr << "Failed\n";
        } else {
            cerr << ans.length() << " moves";
