...
struct State {
    ...
    bool _is_survivable() {
        ...
        while (idx < tail) {
            int curr = queue[idx++];
            reachable++;
            if (reachable >= limit) return true;
    ...
    long long _calc_score() {
        ...
        int panic_limit = 400;
        if (matched < 5) panic_limit = 100;
        else if (matched < 20) panic_limit = 200;
...
string beam_search(int beam_width, int max_turns, double time_limit, int noise_max) {
    ...
    for (int turn = 0; turn < max_turns; ++turn) {
        ...
        if (turn - last_match_turn > 500) break;
        ...
        for (State& state : beam) {
            ...
            for (int d_idx = 0; d_idx < 4; ++d_idx) {
                ...
                if (bite_idx_new != -1) {
                    ...
                    int eaten_color;
                    if (npos == state.snake[k - 1]) eaten_color = 0;
                    else eaten_color = state.board[npos];
                    ...
                    if (eaten_color > 0) {
                        ...
                        if (nxt_state.matched > state.matched) nxt_state.last_match_turn = nxt_state.moves_length;
        ...
        if (best_state.matched == M && best_state.garbage == 0) break;
    }

    if (best_state.matched < M || best_state.garbage > 0) return "";
...
string make_snake_path() {
    ...
    while (c < N - 1) {
        if (direction_down) {
            while (r < N - 1) { moves += 'D'; r += 1; }
        } else {
            while (r > 0) { moves += 'U'; r -= 1; }
        }
        moves += 'R'; c += 1;
...
int main() {
    ...
    for(int p = 0; p < N * N; ++p) {
        pos_r[p] = p / N; pos_c[p] = p % N; adj_len[p] = 0;
        int r = pos_r[p], c = pos_c[p];
        if(r > 0) adj[p][adj_len[p]++] = p - N;
        if(r < N - 1) adj[p][adj_len[p]++] = p + N;
        if(c > 0) adj[p][adj_len[p]++] = p - 1;
        if(c < N - 1) adj[p][adj_len[p]++] = p + 1;
    }
    ...
    for (int p = 0; p < (int)mass_flat.size(); ++p) {
        int c_val = mass_flat[p];
        if (c_val > 0) bait_positions[c_val].push_back(p);
    }
    ...
    if (best_ans.empty()) best_ans = make_snake_path();

    for (char move : best_ans) cout << move << "\n";
