...
int main() {
    ...
    while (true) {
        auto current_time = chrono::high_resolution_clock::now();
        double elapsed_time = chrono::duration<double>(current_time - start_time).count();
        if (elapsed_time > 9.0) break;
        
        double attempt_end_time = elapsed_time + min(0.3, 9.0 - elapsed_time);
        ...
        // --- 追加: ペナルティの重さを 1 〜 4 の間でランダムに決定 ---
        current_penalty_weight = 1 + (mt_engine() % 4);
        ...
        if (!ans.empty() && ans.length() < min_moves) {
            ...
            // --- 追加: ベストスコアを更新した有能なパラメータを出力する ---
            cerr << "Moves: " << min_moves << " | Beam: " << current_beam << " | Noise: " << current_noise << "\n";
            // -----------------------------------------------------------
        }
    }
