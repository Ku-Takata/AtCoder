...
string beam_search(int beam_width, int max_turns, double time_limit, int noise_max) {
    ...
    for (int turn = 0; turn < max_turns; ++turn) {
        ...
        // --- 追加: 早期打ち切り（デススパイラル検知） ---
        if (turn - last_match_turn > 1200) { // 1200ターン餌を食えなかったら見切りをつける
...
int main() {
    ...
    if (probe_ans.empty()) {
        cerr << "Probe Result : Failed (Time out or Dead End)\n";
        cerr << "Difficulty   : HARD\n";
        // 激ムズ用: 全滅を防ぐためにビーム幅を広めに取り、ノイズは広く散らす
        beam_min = 10; beam_max = 50;
        noise_min = 500; noise_max = 2000;
    } else {
        best_ans = probe_ans;
        ...
        if (min_moves < 2000) {
            ...
            // 簡単用: ビーム幅を最大化し、ノイズは抑えて手数を極限まで詰める
            beam_min = 30; beam_max = 50;
            noise_min = 100; noise_max = 500;
        } else {
            cerr << "Difficulty   : NORMAL\n";
            // 標準用: バランスの取れた範囲
            beam_min = 20; beam_max = 40;
            noise_min = 400; noise_max = 1200;
        }
    }
    cerr << "====================================\n\n";
    ...
    while (true) {
        ...
        cerr << "Trial " << trial_count++ << " | Beam: " << current_beam << " | Noise: " << current_noise;
