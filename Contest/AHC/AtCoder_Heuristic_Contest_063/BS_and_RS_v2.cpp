    ...
    long long _calc_score() {
        ...
        for (int p : bait_positions[next_color]) {
            if (board[p] == next_color) {
                ...
                // 実際の距離に「障害物の数 × 2歩分」のペナルティを足したものを仮の距離とする
                int perceived_dist = dist + obstacle_count * 2; 
        ...
        if (garbage > 0) {
            ...
            if (matched >= 2) {
                ...
                int obstacle_count = 0;
                for (int i = 1; i < snake_len; ++i) {
                    if (snake[i] == target_pos) continue; // 噛みちぎる目標自体は障害物として数えない
                    int sr = pos_r[snake[i]];
                    int sc = pos_c[snake[i]];
                    if (min_r <= sr && sr <= max_r && min_c <= sc && sc <= max_c) {
                        obstacle_count++;
                ...
                // こちらも障害物1つにつき2歩分のペナルティ
                int perceived_dist_bite = dist_to_bite + obstacle_count * 2;
...
int main() {
    ...
    string best_ans = "";
    int min_moves = 1000000; // 最短のターン数を記録
    
    while (true) {
        auto current_time = chrono::high_resolution_clock::now();
        double elapsed_time = chrono::duration<double>(current_time - start_time).count();
        if (elapsed_time > 1.8) break;
        
        double attempt_end_time = elapsed_time + min(0.3, 1.8 - elapsed_time);
        
        // --- 変更箇所: 試行ごとにパラメータをランダムに振る ---
        // ビーム幅を 10 〜 50 の間で変動
        int current_beam = 10 + (mt_engine() % 41);
        // ノイズの強さを 100 〜 2000 の間で変動
        int current_noise = 100 + (mt_engine() % 1900);
        
        string ans = beam_search(current_beam, 100000, attempt_end_time, current_noise);
        // --------------------------------------------------------
        
        // 完走しており、かつ今までに見つけたルートより手順が短ければ更新
    ...
    // 全てのリセマラ（試行）で完走できなかった最悪ケースのみ、蛇行ルートを出力
    if (best_ans.empty()) {
