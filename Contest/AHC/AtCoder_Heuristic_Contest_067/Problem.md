...
![example](https://img.atcoder.jp/ahc067/BpPfrS30.gif)

### 問題文

$N\times N$ マスの魔王城がある。一番左上のマスの座標を $(0,0)$ とし、そこから下方向に $i$ マス、右方向に $j$ マス進んだ先のマスの座標を $(i,j)$ とする。

各マスは空きマス `.` または障害物 `#` のいずれかである。また、$N\times N$ の範囲外はすべて障害物であるとみなす。
...
1. 上下左右に隣接する空きマスへ移動する。ただし、現在いるマスと移動先のマスの間に扉があり、かつその扉が閉じている場合、その方向へは移動できない。
2. 現在いるマスにスイッチがある場合、そのスイッチを押す。
...
* $c_i$ は長さ $N$ の文字列であり、その $j$ 文字目 $c_{i,j}$ はマス $(i,j)$ が空きマスの場合 `.`、障害物の場合 `#` である。
* $c_{0,0}=c_{N-1,N-1}=$ `.` である。
* すべての空きマスは、空きマスのみを通って入口 $(0,0)$ から到達可能であることが保証されている。
...
[例を見る](https://img.atcoder.jp/ahc067/BpPfrS30.html?lang=ja&seed=0&output=sample)
...
* [Web版](https://img.atcoder.jp/ahc067/BpPfrS30.html?lang=ja): ローカル版より高性能でアニメーション表示が可能です。
* [ローカル版](https://img.atcoder.jp/ahc067/BpPfrS30.zip): 使用するには[Rust言語](https://www.rust-lang.org/ja)のコンパイル環境をご用意下さい。

* [Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc067/BpPfrS30_windows.zip): Rust言語の環境構築が面倒な方は代わりにこちらをご利用下さい。
