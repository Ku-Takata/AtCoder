...
// 経路復元なしの純粋なターン数計算（枝刈り用の threshold を使用）
...
int main() {
    ...
    // 初期解の貪欲法に「余白」を作る（上限を30枚に制限、敷き詰めなし）
    ...
    while (true) {
        ...
        // ★案2: 50イテレーションごとに、無条件で最新の経路情報を同期する（迷子防止）
        if (iter % 50 == 0) {
        ...
        // ★改善策B: 近傍操作を4つのモードに拡張（0: 扉通常, 1: スイッチ通常, 2: 扉移動, 3: スイッチ移動）
        int mode = engine() % 4;
        bool accept = false;
        int delta = 0;
        ...
        if (mode == 0) {
            ...
            // 2. 扉の移動操作（既存の扉を1枚選び、最新経路周辺の空きマスへ移動させる）
            ...
            if (!valid_move) { // 移動先が塞がっていれば巻き戻してスキップ
            ...
            // 3. スイッチの移動操作（既存のスイッチを1つ選び、最新経路の空きマスへ移動させる）
    ...
    // 残り枠を安全なダミー扉で埋める処理（threshold=1でゴール可能チェック）
    int step_dummy = 0;
