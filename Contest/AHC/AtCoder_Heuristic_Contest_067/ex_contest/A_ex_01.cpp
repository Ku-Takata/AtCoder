...
int main() {
    ...
    for (int step = 0; step < K; ++step) {
        ...
        // 案3：ゴール直前の関所（ボトルネック）を意識し、パスを玉座側（逆順）から走査する
        for (int idx = (int)current_path.size() - 1; idx >= 4; --idx) {
    ...
    // 各種類のスイッチの現在の配置座標を管理 (未配置は {-1, -1})
    vector<Point> switch_pos(K, {-1, -1});
    for (int r = 0; r < N; ++r) {
        for (int l = 0; l < N; ++l) {
            if (switch_map[r][l] != -1) {
                switch_pos[switch_map[r][l]] = {r, l};
            }
        }
    }
    ...
    while (true) {
        ...
        if (mode == 0) {
            ...
            int mode1_sub = engine() % 2;
            int new_s = engine() % K;
            Point prev_ns_pos = switch_pos[new_s];

            if (old_s != -1) {
                if (mode1_sub == 0) {
                    // 削除
                    switch_map[i][j] = -1;
                    switch_pos[old_s] = {-1, -1};
                } else {
                    // 種類変更 (既に new_s が別にあればそこを消して上書き)
                    if (old_s == new_s) continue;
                    if (prev_ns_pos.i != -1) {
                        switch_map[prev_ns_pos.i][prev_ns_pos.j] = -1;
                    }
                    switch_map[i][j] = new_s;
                    switch_pos[old_s] = {-1, -1};
                    switch_pos[new_s] = {i, j};
                }
            } else {
                // 空きマスへの新規追加 (既に new_s が別にあればそこを消して移動させる扱いになる)
                if (prev_ns_pos.i != -1) {
                    switch_map[prev_ns_pos.i][prev_ns_pos.j] = -1;
                }
                switch_map[i][j] = new_s;
            ...
            if (accept) {
                ...
                if (old_s != -1) {
                    if (mode1_sub == 0) {
                        switch_map[i][j] = old_s;
                        switch_pos[old_s] = {i, j};
                    } else {
                        if (old_s != new_s) {
                            switch_map[i][j] = old_s;
                            switch_pos[old_s] = {i, j};
                            if (prev_ns_pos.i != -1) {
                                switch_map[prev_ns_pos.i][prev_ns_pos.j] = new_s;
                            }
                            switch_pos[new_s] = prev_ns_pos;
                        }
                    }
                } else {
                    switch_map[i][j] = -1;
            ...
            if (accept) {
                ...
                switch_pos[target.s] = {target.i, target.j};
            }
        }
    }
    ...
    // スイッチを1種類1個ずつ、計K個になるまで埋める
    vector<int> unused_switches;
    vector<bool> switch_used(K, false);
    for (const auto& s : final_switches) {
        if (s.s >= 0 && s.s < K) {
            switch_used[s.s] = true;
        }
    }
    for (int s = 0; s < K; ++s) {
        if (!switch_used[s]) {
            unused_switches.push_back(s);
        }
    }

    int unused_idx = 0;
    for (int i = 0; i < N && unused_idx < (int)unused_switches.size(); ++i) {
        for (int j = 0; j < N && unused_idx < (int)unused_switches.size(); ++j) {
            if (c[i][j] == '.' && switch_map[i][j] == -1) {
                int s_id = unused_switches[unused_idx];
                switch_map[i][j] = s_id;
                final_switches.push_back({i, j, s_id});
                unused_idx++;
            }
        }
    }
