...
struct Solver {
    ...
    mark_stamp: u32,
    is_scattered_cache: Option<bool>,
}

impl Solver {
    fn is_scattered_pond_map(&mut self) -> bool {
        if let Some(cached) = self.is_scattered_cache {
            return cached;
        }
        let total_pond_cells = 2500 - self.total_grass;
        if (total_pond_cells as f64) < self.params.pond_count_th {
            self.is_scattered_cache = Some(false);
            return false;
        }

        let mut pond_shared_edges = 0;
        for i in 0..self.n {
            for j in 0..self.n {
                if self.grid[i][j] != '.' {
                    for d in 0..4 {
        ...
        let clustering = if total_pond_cells > 0 {
            pond_shared_edges as f64 / total_pond_cells as f64
        } else {
            0.0
        };

        let result = clustering <= self.params.pond_cluster_th;
        self.is_scattered_cache = Some(result);
        result
    }
    ...
    fn calc_adjacency(&mut self, cells: &[(usize, usize)]) -> i32 {
        let s = self.stamp_cells(cells);
        let mut adj = 0;
        for &(x, y) in cells {
            for d in 0..4 {
                ...
                if nx >= 0 && nx < self.n as i32 && ny >= 0 && ny < self.n as i32 {
                    ...
                    if neighbor_id >= 0 {
                        ...
                        if self.grp[nid].t < dep_time {
                            ...
                            if dt > 1200 {
                                suffocate += f64::min(1.0, (dt - 1200) as f64 / 8000.0);
                            }
                        }
                    }
                }
    ...
    fn build_dijkstra_blob_strict(&mut self, sx: usize, sy: usize, p: usize) -> Vec<(usize, usize)> {
        ...
        let is_scattered = self.is_scattered_pond_map();
        let mult = if is_scattered { self.params.p3_relax_mult } else { 1.0 };
        let area_mult = self.params.p3_bbox_area_mult * mult;
        let aspect_mult = self.params.p3_bbox_aspect_mult * mult;
        
        let max_allowed_area = (p as f64 * area_mult + 2.0) as usize;
        let max_aspect_diff = 3.max(((p as f64).sqrt() * aspect_mult) as usize);
                        ...
                        if grid[nxu][nyu] == '#' || own[nxu][nyu] >= 0 {
                            ...
                            if own[nxu][nyu] >= 0 {
                                ...
                                if grp[neighbor_id].t < dep_time {
                                    ...
                                    if dt > 1200 {
                                        suff += f64::min(1.0, (dt - 1200) as f64 / 8000.0);
                                    }
                                }
                            }
                ...
                for r in 0..=self.n - h {
                    for c in 0..=self.n - w {
                        if self.count_blocked(r, c, h, w) == 0 {
                            ...
                            let is_scattered = self.is_scattered_pond_map();
                                let size_bonus = if is_scattered && p <= 60 { 0.5 } else if is_scattered && p >= 90 { -0.5 } else { 0.0 };
                                let score = self.calc_heuristics_score(v, base_cost, comp, adj, tbonus, ef, tb, quick_cap, pipe_bonus, suff, size_bonus);
                            
                            fast_rects.push(FastRect {
                                r, c, w, h, excess: 0, comp, adj, tb, tbonus, ef, suff, score,
                            });
                        }
                    }
                }
            ...
            if !fast_rects.is_empty() {
                ...
                if quick_mode {
                    ...
                    for fr in &fast_rects {
                        let cells = get_rect_cells(fr.r, fr.c, fr.w, fr.h, 0);
                        let cap = if elapsed_now < 1.50 { self.eval_cap(&cells) } else { 0 };
                        let cand = PlaceResult {
                            cells,
                            compact: fr.comp,
                            adj: fr.adj,
                        ...
                        for c in 0..=self.n - w {
                                ...
                                && self.count_blocked(r + h - 1, c, 1, last_w) == 0
                            {
                                ...
                                p2_fast.push(FastRect {
                                    r, c, w, h, excess, comp, adj, tb, tbonus, ef, suff, score,
                                });
                            }
                        }
                    }
            ...
            if !p25_cands.is_empty() {
                p25_cands.sort_unstable_by(|a, b| {
                    let get_score = |r: &PlaceResult| -> f64 {
                        self.calc_heuristics_score(v, base_cost, r.compact, r.adj, r.time_bonus, r.exposed_free, r.tie_breaker, r.max_free_rect, r.pipe_bonus, r.suffocate_penalty, r.pocket_fit)
                    };
...
fn main() {
    ...
    let params = Params {
        ...
        p3_delta_weight: args.get(22).and_then(|s| s.parse().ok()).unwrap_or(8706.685981173852),
        p3_bbox_area_mult: args.get(23).and_then(|s| s.parse().ok()).unwrap_or(1.6964741222985522),
        p3_bbox_aspect_mult: args.get(24).and_then(|s| s.parse().ok()).unwrap_or(0.43160827747545355),
        p3_lookahead_steps: args.get(25).and_then(|s| s.parse().ok()).unwrap_or(3),
        pond_count_th: args.get(26).and_then(|s| s.parse().ok()).unwrap_or(520.0),
        pond_cluster_th: args.get(27).and_then(|s| s.parse().ok()).unwrap_or(2.05),
        p2_th_relaxed: args.get(30).and_then(|s| s.parse().ok()).unwrap_or(0.76),
        p3_relax_mult: args.get(31).and_then(|s| s.parse().ok()).unwrap_or(1.1500000000000001),
    };
    ...
    for turn in 0..m {
        ...
        let size_penalty = if free_rate < 0.30 && p >= 80 { 1.15 } else { 1.0 };
        let current_thresh = if turn >= m - 40 {
            0.0
        } else {
            (0.24 + 0.60 * (1.0 - free_rate)) * p_factor * solver.params.density_reject_mult * size_penalty
        };
                ...
                'mm_end: {
                    ...
                    for w in 1..=n {
                        ...
                        if h >= 1 && h <= n {
                            let excess = h * w - p;
                            if excess < w
                                && (h.max(w) as f64 / h.min(w) as f64) <= 1.6
                            {
                                box_shapes.push((h, w));
                            }
                        }
