...
impl Solver {
    ...
    fn find_best_placement(
        ...
        if !best.cells.is_empty() {
            let vc = v as f64 * best.compact;
            let h = self.params.w_adj * best.adj as f64
                + self.params.w_time * best.time_bonus
                - self.params.w_expose * best.exposed_free as f64
                + self.params.w_zone * best.tie_breaker
                + self.params.w_capacity * (best.max_free_rect as f64)
                + self.params.w_pipe * best.pipe_bonus
                - self.params.w_suffocate * best.suffocate_penalty
                + self.params.w_pocket * best.pocket_fit;
            let bh = base_cost * h;
            let ratio = if vc > 0.0 { bh / vc } else { 0.0 };
            eprintln!("[RATIO_DIAG] P={} | VC={:.1} | BH={:.1} | Ratio={:.4}", p, vc, bh, ratio);
        }
