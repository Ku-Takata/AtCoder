...
impl Solver {
        ...
        for i in 0..self.n {
            ...
            for j in 0..=self.n {
                ...
                while st_len > 0 && h[st[st_len - 1]] >= val {
                    ...
                    // 1. 短辺が狭すぎる（3未満）空き地はP=10以上が入らないので強烈なペナルティ
                    if height.min(width) < 2 {
                        score -= 500;
                    }
                    
                    // 2. 面積が200（最大の案件P=150が余裕で入るサイズ）を超えたら評価を頭打ちにする
                    // これにより「巨大な空き地1つ」よりも「使い勝手の良い空き地複数」が自然と選ばれやすくなる
                    if score > 200 {
    ...
    fn find_best_placement(
        ...
        // 高速化: 配列確保を廃止し、$O(1)$ の数学的境界チェックに変更
        ...
        let get_rect_tb = |r: usize,
         ...
         -> f64 {
            ...
            // 修正: 固定の退去ターンではなく「今からどれくらい長く居座るか(rem_time)」で判定
            // 滞在時間(rem_time)が10000を超えるグループを外側に配置しやすくする
            if rem_time > 10000 {
        ...
        // 提案2: ターン進捗が70%を超えたら、外側ボーナスの実質的な重みを徐々に0に減衰させる
        ...
        let top_k = if quick_mode {
            1
        } else if elapsed_now > 1.65 || time_margin < -0.15 {
            4
        } else if elapsed_now > 1.45 || time_margin < -0.05 {
            8
        ...
        if quick_mode {
            // MultiMove内の仮探索ではなく、本当に時間がなくて(1.82秒超過)
            // 打ち切られた場合のみ、最終防衛ラインとして fallback_bfs を発動する
            if best.cells.is_empty() && self.start_time.elapsed().as_secs_f64() >= 1.82 {
                ...
                if !fallback_cells.is_empty() {
                    ...
                    eprintln!(
                        "[Turn {}] Fallback BFS triggered. Rescued P={} group (Compactness: {:.4}, Time: {} us)",
                        turn, p, comp, t_fallback.elapsed().as_micros()
                    );
                }
            }
            return best;
        ...
        // 通常モード: タイムガード判定 + 十分高精度な配置がある場合のスキップ
        ...
        // Phase 3: ガイド付きBFS
        {
            ...
            'p3_outer: for i in 0..self.n {
                for j in 0..self.n {
                    ...
                    let elapsed_now = self.start_time.elapsed().as_secs_f64();
                    // 改善: 節約した時間を使い、柔軟なDijkstra探索の試行回数をさらに倍増させる
...
fn main() {
    ...
    for turn in 0..m {
        ...
        let current_thresh = if turn >= m - 40 {
            ...
            // fee 6849, score 6836
            (0.24 + 0.60 * (1.0 - free_rate)) * p_factor
        };
        ...
        let try_multimove = result.cells.is_empty()
            || (solver.r <= 0.03 && result.compact < 0.90 && v > 250_000)
            || (result.compact < 0.85 && v > 300_000)
            || (result.compact < 0.90 && v > 600_000);
            ...
            if elapsed < 1.65 {
                'mm_end: {
                    ...
                    for &(h, w) in &box_shapes {
                        ...
                        for r in 0..=n - h {
                            for c in 0..=n - w {
                                if solver.start_time.elapsed().as_secs_f64() > 1.65 {
                                    break 'mm_end;
                                }
                                ...
                                // 改善: 価値が非常に高い、または面積が大きく配置困難な案件の救済措置
                                // 既存グループをより多く退去させてでも強引に場所をこじ開ける
                                // fee 6782, score 6768
                                if v > 300_000 || p >= 50 {
                                    max_inter += 3;
                                }
                                ...
                                // ただし、終盤で時間が切迫している場合は探索爆発を防ぐために絞る
                                if elapsed > 1.60 {
                                    max_inter = max_inter.min(4);
                                }

                                // 改善: 許容する移動コストの割合も、価値が高い案件ほど強気に引き上げる (最大 0.85 まで許容)
