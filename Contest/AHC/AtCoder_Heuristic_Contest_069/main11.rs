...
impl Solver {
    // Phase 2.2: 池内包型 穴あき正方形構築 (Border Line Padding)
    fn build_hole_box_blob(&self, r: usize, c: usize, w: usize, h: usize, p: usize) -> Vec<(usize, usize)> {
        let mut cells = Vec::with_capacity(p * 2);
        
        for i in r..(r + h).min(self.n) {
            for j in c..(c + w).min(self.n) {
        ...
        if (c + w) < self.n {
            for i in r..(r + h).min(self.n) {
                if self.grid[i][c + w] == '.' && self.own[i][c + w] == -1 {
                    cells.push((i, c + w));
                    if cells.len() == p {
                        return cells;
                    }
        ...
        // 下一辺
        if (r + h) < self.n {
            for j in c..(c + w + 1).min(self.n) {
                if self.grid[r + h][j] == '.' && self.own[r + h][j] == -1 {
                    cells.push((r + h, j));
                    if cells.len() == p {
                        return cells;
                    }
        ...
        if r > 0 {
            for j in c..(c + w + 1).min(self.n) {
                if self.grid[r - 1][j] == '.' && self.own[r - 1][j] == -1 {
                    cells.push((r - 1, j));
                    if cells.len() == p {
                        return cells;
                    }
    ...
    fn find_best_placement(
                ...
                for r in 0..=self.n - h {
                    for c in 0..=self.n - w {
                        if self.count_blocked(r, c, h, w) == 0 {
                            ...
                            let score = self.calc_heuristics_score(v, base_cost, comp, adj, tbonus, ef, tb, quick_cap, pipe_bonus, suff, 0.0);
        ...
        // Phase 2.2: 完全統合型 池内包 穴あき正方形検索 (Guarded Hole-Box Search)
        if best.compact < 0.85 && p >= 40 && elapsed_now < 1.50 {
            let t0 = Instant::now();
            let sq = 2.max((p as f64).sqrt().round() as usize);
            let mut box_dims: Vec<(usize, usize)> = Vec::new();
            let max_w = self.n.min(sq + 4);
            for w in sq.saturating_sub(1)..=max_w {
                let h = 2.max((p + w - 1) / w);
                if h <= self.n && (w.max(h) as f64 / w.min(h) as f64) <= 1.4 {
                    box_dims.push((w, h));
                    if w != h {
                        box_dims.push((h, w));
                    }
            ...
            let mut p22_cands: Vec<PlaceResult> = Vec::new();
            for &(bw, bh) in &box_dims {
                ...
                for r in 0..=self.n - bh {
                    for c in 0..=self.n - bw {
                        let free_cnt = self.count_free(r, c, bh, bw);
                        if free_cnt < (p as i32) / 2 || free_cnt == (bh * bw) as i32 {
                            continue;
                        }

                        let reg = self.build_hole_box_blob(r, c, bw, bh, p);
                        if reg.len() == p {
            ...
            if !p22_cands.is_empty() {
                p22_cands.sort_unstable_by(|a, b| {
                    let score_a = self.calc_heuristics_score(v, base_cost, a.compact, a.adj, a.time_bonus, a.exposed_free, a.tie_breaker, 0, a.pipe_bonus, a.suffocate_penalty, a.pocket_fit);
                    let score_b = self.calc_heuristics_score(v, base_cost, b.compact, b.adj, b.time_bonus, b.exposed_free, b.tie_breaker, 0, b.pipe_bonus, b.suffocate_penalty, b.pocket_fit);
                    score_b.total_cmp(&score_a)
                });
                
                let eval_limit = if elapsed_now < 1.45 { top_k.min(6) } else { top_k.min(2) };
                if p22_cands.len() > eval_limit {
                    p22_cands.truncate(eval_limit);
                }

                for cand in &mut p22_cands {
                    if elapsed_now < 1.45 {
