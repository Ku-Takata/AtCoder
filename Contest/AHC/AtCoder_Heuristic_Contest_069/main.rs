    ...
    fn find_best_placement(
        ...
        let eval_approx_rect = |r: usize,
         ...
         -> (i32, i32, f64) {
            ...
            for j in c..c + w {
                eval_cell(r, j, &mut adj, &mut ef, &mut tbonus, &mark);
            }
            if h >= 2 {
                if excess == 0 {
                    let i = r + h - 1;
                    for j in c..c + w {
                        eval_cell(i, j, &mut adj, &mut ef, &mut tbonus, &mark);
                    }
                } else {
                    let i_bot = r + h - 1;
                    for j in c..c + last_w {
                        eval_cell(i_bot, j, &mut adj, &mut ef, &mut tbonus, &mark);
                    }
                    let i_prev = r + h - 2;
                    for j in c + last_w..c + w {
                        eval_cell(i_prev, j, &mut adj, &mut ef, &mut tbonus, &mark);
                    }
                }
            }

            for i in r + 1..r + h - 1 {
                eval_cell(i, c, &mut adj, &mut ef, &mut tbonus, &mark);
                if w >= 2 {
                    eval_cell(i, c + w - 1, &mut adj, &mut ef, &mut tbonus, &mark);
                }
            }
        ...
        // Phase 1: 完全長方形 (w*h == p)
        {
            ...
            for &(w, h) in &dims {
                if self.start_time.elapsed().as_secs_f64() > 1.78 {
                    break;
                }
                ...
                for r in 0..=self.n - h {
                    for c in 0..=self.n - w {
                        if self.count_blocked(r, c, h, w) == 0 {
                            ...
                            let (adj, ef, tbonus) = eval_approx_rect(
            ...
            if !fast_rects.is_empty() {
                ...
                if quick_mode {
                    ...
                    let cand = PlaceResult {
                        ...
                        exposed_free: fr.ef,
                        max_free_rect: 0,
                        pipe_bonus: fr.pipe_bonus,
                    };
                    if is_better(&cand, &best) {
                        best = cand;
                    }
                } else {
                    if fast_rects.len() > 10 {
                        fast_rects.truncate(10);
                    }
        ...
        if quick_mode || elapsed_now > 1.78 || best.compact > 0.85 {
            return best;
        }
        ...
        let elapsed_now = self.start_time.elapsed().as_secs_f64();
        if elapsed_now > 1.78 || best.compact > 0.75 {
            return best;
        }
        ...
        // Phase 3: ガイド付きBFS（連結成分ベース）
        {
            ...
            'p3_outer: for i in 0..self.n {
                for j in 0..self.n {
                    ...
                    comp.sort_unstable_by(|a, b| {
                        ...
                        for d in 0..4 {
                            ...
                            if nxa < 0
                                ...
                                || self.own[nxa as usize][nya as usize] >= 0
                            {
                                adj_a += 1;
                            }
                            ...
                            if nxb < 0
                                ...
                                || self.own[nxb as usize][nyb as usize] >= 0
                            {
                                adj_b += 1;
                            }
                        }
                    ...
                    let elapsed_now = self.start_time.elapsed().as_secs_f64();
                    let max_t = if elapsed_now < 0.80 {
                        128
                    } else if elapsed_now < 1.40 {
                        64
                    } else {
                        32
                    };
...
fn main() {
    ...
    for turn in 0..m {
        ...
        let is_hard_mode = solver.start_time.elapsed().as_secs_f64() >= 1.75;
        let result = solver.find_best_placement(p, rem_time, solver.grp[turn].t, is_hard_mode);
        ...
        if !try_multimove {
            ...
            // ★セーフティネット: 1.50sを超えたら新規MultiMove探索を緊急打ち切り
            if elapsed < 1.50 {
                'mm_end: {
                    ...
                    for w in 1..=n {
                        ...
                        if h >= 1 && h <= n {
                            let excess = h * w - p;
                            if excess < w
                                && (h.max(w) as f64 / h.min(w) as f64) <= 2.2
                            {
                                box_shapes.push((h, w));
                            }
                        }
                    ...
                    for &(h, w) in &box_shapes {
                        ...
                        for r in 0..=n - h {
                            for c in 0..=n - w {
                                if solver.start_time.elapsed().as_secs_f64() > 1.50 {
                                    break 'mm_end;
                                }
                                    ...
                                    for j in c..c + w {
                                        ...
                                        if solver.own[i][j] != -1 {
                                            ...
                                            if !inter_mask[id] {
                                                ...
                                                if intersecting.len() > max_inter || running_move_cost > max_allowed_move_cost {
                                                    ok = false;
                                                    break;
                                                }
                                            }
                                ...
                                if base_expected_fee - running_move_cost <= best_mm.net_gain {
                                    continue;
                                }
                                ...
                                inter_sorted.sort_unstable_by(|&a, &b| {
                                    let val_a = solver.grp[a].v as f64 * solver.r;
                                    let val_b = solver.grp[b].v as f64 * solver.r;
            ...
            if best_mm.net_gain > 0 && !best_mm.icells.is_empty() {
                ...
                for plan in &best_mm.move_plans {
                    ...
                    // ★ デバッグログ: 移動対象グループの詳細（ID, コスト, コンパクト度推移, 旧座標 -> 新座標）を出力
                    eprintln!(
                    ...
                    eprintln!("    From: {:?}", solver.grp[id].cells);
                    eprintln!("    To  : {:?}", plan.1);
    ...
    if elapsed >= 1.50 {
        eprintln!("[WARNING] High Execution Time (>1.5s)! Time Safety Guard Engaged.");
    } else {
        eprintln!("[STATUS] Execution Time SAFE (<1.5s).");
    }
    eprintln!(
        "Map Grass: {}/{} ({:.1}%)",
        solver.total_grass,
