...
# Phase別 件数 & Fee
fn_p1_info = re.compile(r"Phase 1 \(C > 0\.95\)\s*:\s*count=\s*(\d+)\s*\|(?::?\s*avgC=[\d\.]+\s*\|)?\s*fee=\s*(\d+)")
fn_p2_info = re.compile(r"Phase 2 \(C > 0\.85\)\s*:\s*count=\s*(\d+)\s*\|(?::?\s*avgC=[\d\.]+\s*\|)?\s*fee=\s*(\d+)")
fn_p25_info = re.compile(r"Phase 2\.5 \(C > 0\.75\)\s*:\s*count=\s*(\d+)\s*\|(?::?\s*avgC=[\d\.]+\s*\|)?\s*fee=\s*(\d+)")
fn_p3_info = re.compile(r"Phase 3 / MultiMove\s*:\s*count=\s*(\d+)\s*\|(?::?\s*avgC=[\d\.]+\s*\|)?\s*fee=\s*(\d+)")
...
fn_occ = re.compile(r"Occupancy Transition \(%\):\s*(.*)")
...
def run_case(seed):
    ...
    m_occ = fn_occ.search(stderr)
    occ_list = []
    if m_occ:
        occ_list = [float(x) for x in m_occ.group(1).split()]
...
def main():
    ...
    print("--------------------------------------------------")
    buckets_time = [
        ("0.0s - 0.4s", 0.0, 0.4), ("0.4s - 0.6s", 0.4, 0.6), ("0.6s - 0.8s", 0.6, 0.8),
        ("0.8s - 1.0s", 0.8, 1.0), ("1.0s - 1.2s", 1.0, 1.2), ("1.2s - 1.4s", 1.2, 1.4),
        ("1.4s - 1.6s", 1.4, 1.6), ("1.6s - 1.85s", 1.6, 1.85), ("1.85s - 2.05s (Guard)", 1.85, 2.05)
    ]
    ...
    print("\n--------------------------------------------------")
    print("   8. TIME GUARD TRIGGERED SEEDS (>= 1.85s)")
    print("--------------------------------------------------")
    tg_cases = [r for r in results if r["time"] >= 1.85]
    ...
    print("\n--------------------------------------------------")
    print("   9. MAP OCCUPANCY TRANSITION (Average %)")
    print("--------------------------------------------------")
    occ_matrix = [r["occ"] for r in results if r.get("occ")]
    if occ_matrix and len(occ_matrix[0]) == 10:
        avg_occ = [mean([row[i] for row in occ_matrix]) for i in range(10)]
        print(f"{'Turn Range':<15} | {'Avg Occupancy':<15} | {'Chart'}")
        print("-" * 70)
        for i in range(10):
            bar = "#" * int(avg_occ[i] / 2)
            print(f"{i*100:03d} - {i*100+99:03d} | {avg_occ[i]:5.1f}%          | {bar}")


    # =====================================================================
    # --- 新設セクション 10: 草の数(池の多さ)によるケース分類分析 ---
    # =====================================================================
    print("\n==================================================")
    print("   10. MAP CHARACTERISTIC ANALYSIS (by Pond Density)")
    print("==================================================")
    print("Categorized cases into 3 groups based on Map Grass (fewer grass = more ponds).")
    
    grass_list = sorted([r["grass"] for r in results])
    if grass_list:
        p33 = grass_list[int(len(grass_list) * 0.33)]
        p67 = grass_list[int(len(grass_list) * 0.67)]
        
        groups_grass = {
            f"Many Ponds (Grass <= {p33})": [r for r in results if r["grass"] <= p33],
            f"Medium Ponds ({p33} < Grass <= {p67})": [r for r in results if p33 < r["grass"] <= p67],
            f"Few Ponds (Grass > {p67})": [r for r in results if r["grass"] > p67],
        }
        
        for g_name, g_results in groups_grass.items():
            if not g_results: continue
            cnt = len(g_results)
            g_scores = [r["score"] for r in g_results]
            g_times = [r["time"] for r in g_results]
            g_acc = [sum(r["buckets"].get(b, {}).get("acc", 0) for b in r["buckets"]) for r in g_results]
            
            print(f"\n[{g_name}] - {cnt} cases")
            print(f"  Avg Score     : {mean(g_scores):,.2f}")
            print(f"  Avg Accept    : {mean(g_acc):.1f} groups/case")
            print(f"  Avg Exec Time : {mean(g_times):.3f} s")
            
            g_occ_matrix = [r["occ"] for r in g_results if r.get("occ")]
            if g_occ_matrix and len(g_occ_matrix[0]) == 10:
                g_avg_occ = [mean([row[i] for row in g_occ_matrix]) for i in range(10)]
                # 2段に分けて見やすく出力
                occ_str_1 = " | ".join([f"T{i*100:03d}: {g_avg_occ[i]:.1f}%" for i in range(5)])
                occ_str_2 = " | ".join([f"T{(i+5)*100:03d}: {g_avg_occ[i+5]:.1f}%" for i in range(5)])
                print(f"  Avg Occupancy :")
                print(f"    {occ_str_1}")
                print(f"    {occ_str_2}")

    # =====================================================================
    # --- 新設セクション 11: 移動コスト(R)によるケース分類分析 ---
    # =====================================================================
    print("\n==================================================")
    print("   11. MAP CHARACTERISTIC ANALYSIS (by Move Cost 'R')")
    print("==================================================")
    print("Categorized cases into 3 groups based on the move cost multiplier (R).")
    
    r_list = sorted([r["r"] for r in results])
    if r_list:
        r33 = r_list[int(len(r_list) * 0.33)]
        r67 = r_list[int(len(r_list) * 0.67)]
        
        groups_r = {
            f"Low Move Cost (R <= {r33:.4f})": [res for res in results if res["r"] <= r33],
            f"Medium Move Cost ({r33:.4f} < R <= {r67:.4f})": [res for res in results if r33 < res["r"] <= r67],
            f"High Move Cost (R > {r67:.4f})": [res for res in results if res["r"] > r67],
        }
        
        for g_name, g_results in groups_r.items():
            if not g_results: continue
            cnt = len(g_results)
            g_scores = [res["score"] for res in g_results]
            g_times = [res["time"] for res in g_results]
            g_acc = [sum(res["buckets"].get(b, {}).get("acc", 0) for b in res["buckets"]) for res in g_results]
            ...
            g_occ_matrix = [res["occ"] for res in g_results if res.get("occ")]
            if g_occ_matrix and len(g_occ_matrix[0]) == 10:
                g_avg_occ = [mean([row[i] for row in g_occ_matrix]) for i in range(10)]
                occ_str_1 = " | ".join([f"T{i*100:03d}: {g_avg_occ[i]:.1f}%" for i in range(5)])
                occ_str_2 = " | ".join([f"T{(i+5)*100:03d}: {g_avg_occ[i+5]:.1f}%" for i in range(5)])
