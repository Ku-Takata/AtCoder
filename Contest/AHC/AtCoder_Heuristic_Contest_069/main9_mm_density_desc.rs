                        ...
                        for r in 0..=n - h {
                            for c in 0..=n - w {
                                ...
                                inter_sorted.sort_unstable_by(|&a, &b| {
                                    let da = solver.grp[a].v as f64 / (solver.grp[a].p as f64 * (solver.grp[a].t - solver.grp[a].s) as f64);
                                    let db = solver.grp[b].v as f64 / (solver.grp[b].p as f64 * (solver.grp[b].t - solver.grp[b].s) as f64);
                                    db.total_cmp(&da)
                                });
