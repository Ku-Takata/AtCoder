...
def run_experiment(mode, num_seeds=1000):
    print(f'=== Running Mode: {mode} on {num_seeds} seeds ===', flush=True)
    tasks = [(s, mode) for s in range(num_seeds)]
    results = []
    t0 = time.time()
    ...
    print(f'Mode: {mode:12s} | Time: {elapsed:5.1f}s | Avg Score: {avg_score:10.1f} | Avg Fee: {avg_fee:10.1f} | Avg Accepts: {avg_accepts:5.1f} | Avg Move Cost: {avg_move_cost:8.1f}', flush=True)
    return {
        'mode': mode, 'avg_score': avg_score, 'avg_fee': avg_fee,
        'avg_accepts': avg_accepts, 'avg_moves': avg_moves, 'avg_move_cost': avg_move_cost
    }

if __name__ == '__main__':
    run_experiment('full', num_seeds=1000)
    run_experiment('pure_c_max', num_seeds=1000)
