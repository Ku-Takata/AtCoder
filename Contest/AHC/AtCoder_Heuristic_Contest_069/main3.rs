...
impl Solver {
    ...
    fn find_best_placement(
        ...
        // Phase 1: 完全長方形 (w*h == p)
        {
            ...
            if !fast_rects.is_empty() {
                ...
                if quick_mode {
                    ...
                    let eval_limit = if elapsed_now < 1.55 { top_k.min(8) } else { top_k.min(3) };
                    if fast_rects.len() > eval_limit {
                        fast_rects.truncate(eval_limit);
                    }
                    for fr in &fast_rects {
                        let cells = get_rect_cells(fr.r, fr.c, fr.w, fr.h, 0);
                        let cap = if elapsed_now < 1.55 { self.eval_cap(&cells) } else { 0 };
                        let cand = PlaceResult {
                            cells,
                            compact: fr.comp,
                            adj: fr.adj,
        ...
        // 修正: quick_mode の場合は Phase 1/2 の結果で即座に帰還 (MultiMoveの爆発を防ぐ)
        if quick_mode {
            return best;
        }
        ...
        if (elapsed_now > 1.82 || best.compact > p2_skip_thresh) && !best.cells.is_empty() {
            return best;
        }
        ...
        // Phase 2.5: Box内BFS
        {
            ...
            box_dims.dedup();
            let max_boxes = if time_margin < -0.05 { 8 } else { 15 };
        ...
        if (elapsed_now > 1.82 || best.compact > p25_skip_thresh) && !best.cells.is_empty() {
            return best;
        }
        ...
        // Phase 3: ガイド付きBFS
        {
            ...
            'p3_outer: for i in 0..self.n {
                for j in 0..self.n {
                    ...
                    let elapsed_now = self.start_time.elapsed().as_secs_f64();
                    // 未配置(best.cellsが空)なら、1.80s超えの超終盤でも数回のスタート地点で粘る
                    let max_t = if elapsed_now > 1.80 {
                        if best.cells.is_empty() { 3 } else { 1 }
                    } else if elapsed_now > 1.60 || time_margin < -0.15 {
                        24
                    } else if elapsed_now > 1.40 || time_margin < -0.05 {
                        48
                    } else if time_margin > 0.10 {
                        128
                    } else {
