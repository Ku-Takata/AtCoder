                    ...
                    for c in 0..=self.n - w {
                        if self.count_blocked(r, c, h, w) == 0 {
                            ...
                            let is_scattered = self.is_scattered_pond_map();
                                ...
                                let score = self.calc_heuristics_score(v, base_cost, comp + size_bonus, adj, adj_pond, adj_border, tbonus, ef, tb, quick_cap, pipe_bonus, suff, 0.0);
...
fn main() {
    ...
    let params = Params {
        ...
        pond_count_th: args.get(26).and_then(|s| s.parse().ok()).unwrap_or(500.0),
        pond_cluster_th: args.get(27).and_then(|s| s.parse().ok()).unwrap_or(1.80),
    };
