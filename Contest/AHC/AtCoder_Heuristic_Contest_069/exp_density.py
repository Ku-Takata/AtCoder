...
def run_experiment(mult, num_seeds=100):
    print(f'=== Testing density_reject_mult = {mult} on {num_seeds} seeds ===', flush=True)
    ...
    print(f'mult={mult:.1f} | Time: {elapsed:.2f}s | Avg Score: {avg_score:.1f} | Avg Fee: {avg_fee:.1f} | Avg Accepts: {avg_accepts:.1f} | Avg Move Cost: {avg_move_cost:.1f}', flush=True)
    return {
...
if __name__ == '__main__':
    for m in [1.0, 0.5, 0.0, 1.5]:
        run_experiment(m, num_seeds=100)
