                ...
                for r in 0..=self.n - h {
                    for c in 0..=self.n - w {
                        if self.count_blocked(r, c, h, w) == 0 {
                            ...
                            let score = (v as f64 * comp) + (base_cost * heuristics);
            ...
            if !p25_cands.is_empty() {
                p25_cands.sort_unstable_by(|a, b| {
                    let get_score = |r: &PlaceResult| -> f64 {
                        let heuristics = self.params.w_adj * r.adj as f64
                            + self.params.w_time * r.time_bonus
                            - self.params.w_expose * r.exposed_free as f64
                            + self.params.w_zone * r.tie_breaker
                            + self.params.w_pipe * r.pipe_bonus
                            - self.params.w_suffocate * r.suffocate_penalty
                            + self.params.w_pocket * r.pocket_fit;
...
fn main() {
    ...
    let params = Params {
        ...
        p3_delta_weight: args.get(22).and_then(|s| s.parse().ok()).unwrap_or(7498.415168333104),
        p3_bbox_area_mult: args.get(23).and_then(|s| s.parse().ok()).unwrap_or(1.4815705068547589),
        p3_bbox_aspect_mult: args.get(24).and_then(|s| s.parse().ok()).unwrap_or(0.5104912608310379),
    };
