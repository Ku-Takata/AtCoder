...
def run_single_case(args):
    ...
    return {
        ...
        'accepts': accepts, 'p3_count': p3, 'p3_fee': fee3,
        'moves': moves, 'move_cost': move_cost
    }

def run_experiment(w_delta, num_seeds=1000):
    print(f'=== Testing p3_delta_weight = {w_delta} on {num_seeds} seeds ===', flush=True)
    tasks = [(s, w_delta) for s in range(num_seeds)]
    results = []
    t0 = time.time()
    ...
    avg_p3_count = sum(r['p3_count'] for r in results) / len(results)
    avg_p3_fee = sum(r['p3_fee'] for r in results) / len(results)
    avg_move_cost = sum(r['move_cost'] for r in results) / len(results)
    
    print(f'weight={w_delta:7.1f} | Time: {elapsed:5.1f}s | Avg Score: {avg_score:10.1f} | Avg Fee: {avg_fee:10.1f} | Avg P3 Fee: {avg_p3_fee:9.1f} (count={avg_p3_count:4.1f}) | Avg Move Cost: {avg_move_cost:8.1f}', flush=True)
    return {
        'weight': w_delta, 'avg_score': avg_score, 'avg_fee': avg_fee,
        'avg_accepts': avg_accepts, 'avg_p3_count': avg_p3_count, 'avg_p3_fee': avg_p3_fee,
        'avg_move_cost': avg_move_cost
    }

if __name__ == '__main__':
    weights = [10000.0, 2000.0, 500.0, 100.0, 20.0, 0.0]
    for w in weights:
        run_experiment(w, num_seeds=1000)
