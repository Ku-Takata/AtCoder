...
// 案2: ジグザグ階段のスムージング（垂直化）
// 左右に振動しながら登っているレンガを、垂直な柱（幅1）+必要な段のみ幅広レンガに置き換える
void smooth_zigzags(vector<Brick>& bricks, const vector<pair<int, int>>& ab,
                    const map<int, int>& cost, int W, int H, int K) {
    ...
    while (changed) {
        ...
        // 各レンガ b について、もし幅が 3 以上で、親 p と子 c があり、
        // 親と子の中心が同じ x 座標で、自身だけが横に 1 マスはみ出している場合（左右の揺れ）
        for (int i = 0; i < n; ++i) {
            if (bricks[i].y == 0 || bricks[i].y + 1 >= H) continue;
            int mid_x = bricks[i].x + (bricks[i].l - 1) / 2;
            int y = bricks[i].y;

            int p = cell_brick[mid_x][y - 1];
            if (p == -1) continue;

            // 直上の子を探す（自身の中央マス mid_x の真上を支持している子、あるいは端の上に乗っている子）
            // 自身を中心とする垂直ライン target_x
            for (int target_x : {mid_x - 1, mid_x + 1}) {
                if (target_x < 0 || target_x >= W) continue;
                // 段 y-1 に target_x を覆うレンガがあるか？
                int p_alt = cell_brick[target_x][y - 1];
                if (p_alt == -1) continue;

                // 自身を target_x を中心とする幅1のレンガにシフトできるか？
                // または target_x を中心とする幅3のレンガにシフトできるか？
                for (int L_new : {1, 3}) {
                    int bx = target_x - (L_new - 1) / 2;
                    int rx = bx + L_new - 1;
                    ...
                    bool conflict = false;
                    for (int x = bx; x <= rx; ++x) {
                        if (cell_brick[x][y] != -1 && cell_brick[x][y] != i) {
                            conflict = true;
                            break;
                        }
                    }
...
// 枝切断＆合流最適化（Branch Cut & Reconnect）
...
vector<Brick> run_dijkstra_fast(int W, int H, int K, const map<int, int>& cost, 
    ...
    smooth_zigzags(ans, ab, cost, W, H, K);
    prune_solution(ans, ab, cost, W, H, K);
    return ans;
}

// 案1: 焼きなまし法（Simulated Annealing）による局所探索
void simulated_annealing(vector<Brick>& cur_bricks, const vector<pair<int, int>>& ab,
    ...
    int cur_cost = calc_total_cost(cur_bricks);
    ...
    while (true) {
        ...
        auto now = chrono::steady_clock::now();
        double el = chrono::duration<double, milli>(now - start_time).count();
        double time_ratio = min(1.0, el / time_limit_ms);
        // 指数冷却
        double T = T_start * pow(T_end / T_start, time_ratio);
        ...
        smooth_zigzags(next_bricks, ab, cost, W, H, K);
        prune_solution(next_bricks, ab, cost, W, H, K);

        int next_cost = calc_total_cost(next_bricks);
        int delta = next_cost - cur_cost;
...
int main() {
    ...
    smooth_zigzags(best_ans, ab, cost, W, H, K);
    prune_solution(best_ans, ab, cost, W, H, K);

    // 3. 焼きなまし法（1650msで確実に終了、TLE完全防止）
    simulated_annealing(best_ans, ab, cost, is_hole, W, H, K, start_time, 1650.0);
