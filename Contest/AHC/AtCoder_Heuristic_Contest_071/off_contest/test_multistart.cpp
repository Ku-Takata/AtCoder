...
// 乱数付きダイクストラ法（多様な木を高速生成）
vector<Brick> run_randomized_dijkstra(int W, int H, int K, const map<int, int>& cost, 
                                      const vector<pair<int, int>>& ab, 
                                      const vector<vector<bool>>& is_hole,
                                      int hole_bonus, double noise_scale) {
...
int main() {
    ...
    for (int iter = 0; iter < 50; ++iter) {
        int bonus = rng() % 8;
        double noise = (iter == 0 ? 0.0 : 1.5);
        auto ans = run_randomized_dijkstra(W, H, K, cost, ab, is_hole, bonus, noise);
        int tot = 0;
        for (const auto& b : ans) tot += cost[b.l];
        min_c = min(min_c, tot);
        cerr << "Iter " << iter << ": Cost=" << tot << " (Best=" << min_c << ")" << endl;
    }
    return 0;
}
