...
vector<Brick> run_test(int W, int H, int K, const map<int, int>& cost, 
                       const vector<pair<int, int>>& ab, 
                       const vector<vector<bool>>& is_hole,
                       int floor_penalty, int hole_bonus) {
    ...
    while (remaining_holes > 0) {
        ...
        int f_cost = (ans.empty() ? 0 : floor_penalty);
        for (int x = 0; x < W; ++x) {
            if (!grid[x][0]) {
                dist_buf[x][0] = f_cost;
...
int main() {
    ...
    for (int penalty : {0, 20, 40, 60, 80, 100, 150}) {
        auto ans = run_test(W, H, K, cost, ab, is_hole, penalty, 4);
        int tot = 0;
        int floor_roots = 0;
        for (const auto& b : ans) {
            tot += cost[b.l];
            if (b.y == 0) floor_roots++;
        }
        cerr << "Penalty=" << penalty << ": Cost=" << tot << ", Floor Roots=" << floor_roots << endl;
    }
    return 0;
}
